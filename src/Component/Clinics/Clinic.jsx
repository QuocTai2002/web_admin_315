// Trong component Clinic
import React, { useState } from "react";
import { Modal, Breadcrumb, Input } from "antd";
  ...
  return (
    <div className="mx-20">
      ...
      <div
        ...
        className="bg-white flex text-center mt-5 rounded-lg "
      >
        <div className="w-1/6 xl:w-1/5 flex flex-col mt-2 item-center p-8 xl:p-6 border-transparent border border-solid border-[#e5eaef]">
          ...
          <ul className="p-0 text-left">
            {chuyenkhoa?.map((items) => (
              <li
                onClick={() => {
                  setIsClassActive(items);
                }}
                className={`list-none px-5 py-3 rounded-md my-3 cursor-pointer hover:bg-[#f6f9fc] ${
                  isClassActive === items && "bg-[#f2f6ff]"
                }`}
              >
                {items}
              </li>
        ...
        <div className="w-5/6 xl:w-4/5 p-8 border-[#e5eaef] border border-solid text-left">
          ...
          />
          <div className="mt-4">
            <TableClinic onOpenDetail={openDetailModal} />
          </div>
        </div>
      </div>
      <AddBranchModal chuyenKhoa={chuyenkhoa} />
      <ModalDelete />
      <ModalDetail selectedBranch={selectedBranch} onCloseDetailModal={onCloseDetailModal} />
    </div>
  );
};
